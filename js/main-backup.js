...
// --- Interactive Video Poster Logic (UPDATED) ---
    ...
    interactiveVideos.forEach(container => {
        ...
        if (video && controlButton) {
            ...
            //    The video element has no 'src' at this point, preventing any preloading.
            const videoSrc = video.dataset.src; // CHANGED: Read from data-src
            if (videoSrc) {
                const posterUrl = videoSrc.substring(0, videoSrc.lastIndexOf('.')) + '.avif';
                video.poster = posterUrl;
            }

            // This function sets up the final controls once metadata is known (after the first play).
            const setupFinalControls = () => {
                ...
                // B. For short videos (< 30s), set up our custom play/pause UI updates.
            ...
            // This universal click handler now manages setting the src on first click.
            controlButton.addEventListener('click', (e) => {
                e.stopPropagation();

                if (video.paused) {
                    // --- NEW: Load the video source on the first click ---
                    ...
                    // as long videos will switch to native browser controls.
                    video.pause();
                }
            });
        }
    ...
    const headerVideoBases = ['je-inflate', 'je-fluid', 'je-molecular', 'je-twist', 'je-blocks'];
